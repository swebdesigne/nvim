...
### Горячие клавиши Goto Preview
| Клавиша | Описание |
|---------|----------|
| `gpd` | Просмотр определения (Definition) в плавающем окне |
| `gpi` | Просмотр реализации (Implementation) в плавающем окне |
| `gpD` | Просмотр объявления (Declaration) в плавающем окне |
| `gpt` | Просмотр определения типа (Type Definition) в плавающем окне |
| `gpr` | Просмотр всех ссылок (References) через Telescope |
| `gpc` | Закрыть все плавающие окна goto-preview |
| `gps` | Просмотр определения в вертикальном сплите |
